...
out <- prepDocuments(processed$documents, processed$vocab, processed$meta, lower.thres = 5)#, subsample = 20000) # TODO: Remove subsample when complete
```
...
thoughts1 <- findThoughts(fit, texts =as.character(d[-removed,]$cleaned_text), topic=30, n = 4)
print(thoughts1)
...
# TODO: Select 4 topics of interest. Topic 4 contains a lot of AAE cursing, topic 13 is the most associated with abusive (seems odd). 20 (feelings) is least abusive, 15 is most neutral (thinking about world). 2 is quite clear, it is about international conflict and is quite hateful. 9 is interesting, possibly song lyrics.
```

```{r content, echo=TRUE}
# Now how does topic content vary?
plot(fit, type="perspectives", topics=c(4,14))
