...
a <- ggplot(effects %>% filter(topic == 4), aes(x = value, y = proportion,
  ...
  geom_line() + labs(x="", y="Topic proportion", fill = "", title="Topic 4: ass, bitch, nigga, yall, ugli") +
  geom_ribbon(aes(ymin=lower, ymax=upper), alpha=0.8) + theme_bw() + theme(legend.position = "none",plot.title = element_text(size=10), axis.title.x = element_text(size=10), axis.title.y = element_text(size=10))+ ylim(-0.05,0.5) + xlim(0,1)

b <- ggplot(effects %>% filter(topic == 20), aes(x = value, y = proportion,
                   group = moderator,
                   fill = factor(moderator))) +
  geom_line() + labs(x="", y="", fill = "", title="Topic 20: fuck, asshol, cunt, slut, bullshit") +
  geom_ribbon(aes(ymin=lower, ymax=upper), alpha=0.8) + theme_bw() + theme(legend.position = "none",plot.title = element_text(size=10), axis.title.x = element_text(size=10), axis.title.y = element_text(size=10))+ ylim(-0.05,0.5) + xlim(0,1)
